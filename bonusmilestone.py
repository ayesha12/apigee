...
@app.route('/stats/cpu')
def api_stats_cpu():
	return api_finalstat("cpu")

@app.route('/stats/memory')
def api_stats_memory():
        return api_finalstat("memory")

@app.route('/stats/disk')
def api_stats_disk():
        return api_finalstat("disk")



@app.route('/stats')
def api_stats(pName):
	return api_finalstat("")

def api_finalstat(pName):
	print pName
	...
	cstart = rs["cpustart"]
	if pName == "cpu" :
		res = {"cpu": { "count": ccount ,"timeRange": {"start": cstart,"end": rs["cpuend"]},"maxValue": rs["cpumaxValue"],"minValue": rs["cpuminValue"],"averageValue": rs["cpuaverageValue"],"99thPercentile": rs["cpu99thPercentile"]}}
	elif pName == "memory" :
		res = {"memory": { "count": rs["memcount"],"timeRange": {"start": rs["memstart"],"end": rs["memend"]},"maxValue": rs["memmaxValue"],"minValue": rs["memminValue"],"averageValue": rs["memaverageValue"],"99thPercentile": rs["mem99thPercentile"]}}
	elif pName == "disk" :
		res = { "disk": { "count": rs["dcount"],"timeRange": {"start": rs["dstart"],"end": rs["dend"]},"maxValue": rs["dmaxValue"],"minValue": rs["dminValue"],"averageValue": rs["daverageValue"],"99thPercentile": rs["d99thPercentile"]}}
	else :
